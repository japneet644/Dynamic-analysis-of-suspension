function [X] = front_force_distriution_in_connected_links()
...
%%% f1l*cos(theta1) + f2l*cos(theta2) = f1a
%%% -f1l*sin(theta1) + f2l*sin(theta2) = f1b
...
g1 = [100 200 250];
g2 = cross(vec4,vec5)/ norm(cross(vec4,vec5));
for i = 1:5 
...
upv = (pc4 - pc5)/norm(pc4 - pc5) ; %upperlinkvector
k1 = (p3 - pc4)/ norm(p3 - pc4); %pull rod force point of application to torque point3

pullrod_force(i,:) = -(new_forces_value(i,3)*dot(upv,cross(vec4,g2),3))/ dot(upv,cross(k1,vec3)); % magnitude only
%%% torque balancing about the upv vector

end

theta3 = acos(dot(vec4,f1x));
theta4 = acos(dot(vec5,f1x));

mat4 = [cos(theta3)  sin(theta4);
        cos(theta4)  sin(theta4);];

for i = 1:5
    F_u = [ X(i,1)   new_forces_value(i,2);
          ];
    upper_f(i,:) = F_u * pinv(mat4);
end  
 disp('upper_f is')
  disp(upper_f)
   disp('lower_f is')
